...
class SQLServer {
    constructor(apiGateway, options) {
        ...
        if ((0, shared_1.getEnv)('nativeApiGateway')) {
    ...
    async execSql(sqlQuery, stream, securityContext) {
        await (0, native_1.execSql)(this.sqlInterfaceInstance, sqlQuery, stream, securityContext);
    }
    buildCheckSqlAuth(options) {
    ...
    async init(options) {
        ...
        const checkSqlAuth = this.buildCheckSqlAuth(options);
    ...
    async shutdown(mode) {
        await (0, native_1.shutdownInterface)(this.sqlInterfaceInstance, mode);
    }
}
