...
class MysqlFilter extends BaseFilter_1.BaseFilter {
...
class MysqlQuery extends BaseQuery_1.BaseQuery {
    newFilter(filter) {
    ...
    convertTz(field) {
        return `CONVERT_TZ(${field}, @@session.time_zone, '${(0, moment_timezone_1.default)().tz(this.timezone).format('Z')}')`;
    }
    timeStampCast(value) {
    ...
    preAggregationTableName(cube, preAggregationName, skipSchema) {
        ...
        if (name.length > 64) {
            throw new UserError_1.UserError(`MySQL can not work with table names that longer than 64 symbols. Consider using the 'sqlAlias' attribute in your cube and in your pre-aggregation definition for ${name}.`);
        }
        return name;
    }
    sqlTemplates() {
...
//# sourceMappingURL=MysqlQuery.js.map