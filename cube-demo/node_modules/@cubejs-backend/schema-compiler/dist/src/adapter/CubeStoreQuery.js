...
class CubeStoreFilter extends BaseFilter_1.BaseFilter {
...
class CubeStoreQuery extends BaseQuery_1.BaseQuery {
    newFilter(filter) {
        return new CubeStoreFilter(this, filter);
    }
    convertTz(field) {
        return `CONVERT_TZ(${field}, '${(0, moment_timezone_1.default)().tz(this.timezone).format('Z')}')`;
    }
    timeStampParam() {
        return 'to_timestamp(?)';
    }
    ...
    regularAndTimeSeriesRollupQuery(regularMeasures, multipliedMeasures, cumulativeMeasures, preAggregationForQuery) {
        ...
        const filters = this.segments.concat(this.filters).concat(timeDimension?.dateRange && this.dateFromStartToEndConditionSql(commonDateCondition, true, true) || []);
        ...
        return this.evaluateSymbolSqlWithContext(() => this.overTimeSeriesSelectRollup(cumulativeMeasuresWithoutMultiplied, regularMeasures.concat(multipliedMeasures), this.evaluateSymbolSqlWithContext(() => this.preAggregations?.rollupPreAggregation(preAggregationForQuery, allMeasures, false, filters), {
            granularityOverride,
            overTimeSeriesAggregate: true
        }), baseQueryAlias, timeDimensionWithGranularity, preAggregationForQuery), {
            wrapQuery: true,
