...
class BaseQuery {
    ...
    rollingWindowDateJoinCondition(trailingInterval, leadingInterval, offset) {
        offset = offset || 'end';
        return this.timeDimensions.map(d => [d, (dateFrom, dateTo, dateField, dimensionDateFrom, dimensionDateTo, isFromStartToEnd) => {
    ...
    fullKeyQueryAggregateMeasures(context) {
        ...
        const withQueries = [];
        const postAggregateMembers = ramda_1.default.uniq(this.allMembersConcat(false)
            // TODO boolean logic filter support
            .filter(m => m.expressionPath && hasPostAggregateMembers(m.expressionPath()))
            .map(m => m.expressionPath())).map(m => this.postAggregateWithQueries(m, {
    ...
    collectAllPostAggregateMembers(allMemberChildren) {
        ...
        return ramda_1.default.fromPairs(allMembers.map(m => ([m, this.memberInstanceByPath(m).isPostAggregate()])));
    }
    ...
    postAggregateWithQueries(member, queryContext, memberChildren, withQueries) {
        ...
        memberFrom = memberChildren[member] &&
            ramda_1.default.uniqBy(f => f.alias, memberChildren[member].map(child => this.postAggregateWithQueries(child, this.childrenPostAggregateContext(member, unionDimensionsContext, wouldNodeApplyFilters), memberChildren, withQueries)));
    ...
    granularityHierarchies() {
        return ramda_1.default.fromPairs(Object.keys(standardGranularitiesParents).map(k => [k, this.granularityParentHierarchy(k)]));
    }
    ...
    overTimeSeriesForSelect(cumulativeMeasures) {
        return this.dimensions.map(s => s.cumulativeSelectColumns()).concat(this.dateSeriesSelect()).concat(cumulativeMeasures.map(s => s.cumulativeSelectColumns())).filter(c => !!c)
            .join(', ');
    }
    dateSeriesSelect() {
    ...
    collectRootMeasureToHieararchy(context) {
        ...
        return ramda_1.default.fromPairs(this.measures.concat(notAddedMeasureFilters).map(m => {
            ...
            if (m.expressionName && !collectedMeasures.length && !m.isMemberExpression) {
                throw new UserError_1.UserError(`Subquery dimension ${m.expressionName} should reference at least one measure`);
            }
    ...
    joinSql(toJoin) {
        ...
        const joins = rest.map(j => `LEFT JOIN ${j.sql} ${this.asSyntaxJoin} ${j.alias} ON ${j.on}`);
    ...
    checkShouldBuildJoinForMeasureSelect(measures, keyCubeName) {
        return measures.map(measure => {
            ...
            if (ramda_1.default.any(cubeName => keyCubeName !== cubeName, cubes)) {
                const measuresJoin = this.joinGraph.buildJoin(joinHints);
                if (measuresJoin.multiplicationFactor[keyCubeName]) {
                    throw new UserError_1.UserError(`'${measure.measure}' references cubes that lead to row multiplication. Please rewrite it using sub query.`);
                }
                return true;
            }
            return false;
    ...
    keyDimensions(primaryKeyDimensions) {
        ...
        return ramda_1.default.uniqBy((d) => `${d.dimension}${d.granularity ?? ''}`, this.dimensionsForSelect()
    ...
    getFieldIndex(id) {
        ...
        let index;
        index = this.dimensionsForSelect().findIndex(d => equalIgnoreCase(d.dimension, id) || equalIgnoreCase(d.expressionName, id));
        if (index > -1) {
            return index + 1;
        }
    ...
    renderSqlMeasure(name, evaluateSql, symbol, cubeName, parentMeasure, orderBySql) {
        ...
        if (symbol.postAggregate) {
            ...
            if (!(ramda_1.default.equals(this.postAggregateDimensions.map(d => d.expressionPath()), this.dimensions.map(d => d.expressionPath())) &&
                ramda_1.default.equals(this.postAggregateTimeDimensions.map(d => d.expressionPath()), this.timeDimensions.map(d => d.expressionPath())))) {
                let funDef;
        ...
        return this.cacheValue(['preAggregationInvalidateKeyQueries', cube, JSON.stringify(preAggregation)], () => {
            ...
            if (preAggregation.refreshKey) {
                if (preAggregation.refreshKey.sql) {
                    return [
                        this.paramAllocator.buildSqlAndParams(preAggregationQueryForSql.evaluateSql(cube, preAggregation.refreshKey.sql)).concat({
                            external: false,
