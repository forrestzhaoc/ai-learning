...
class PreAggregations {
    ...
    preAggregationDescriptionsFor(foundPreAggregation) {
        ...
        return preAggregations.map(preAggregation => {
            if (this.canPartitionsBeUsed(preAggregation)) {
                ...
                return this.preAggregationDescriptionsForRecursive(preAggregation.cube, this.addPartitionRangeTo(preAggregation, dimension, partitionDimension.wildcardRange(), partitionDimension.boundaryDateRange || partitionDimension.dateRange));
            }
    ...
    findPreAggregationToUseForCube(cube) {
        ...
        const originalSqlPreAggregations = ramda_1.default.pipe(ramda_1.default.toPairs, ramda_1.default.filter(([, a]) => a.type === 'originalSql'))(preAggregates);
    ...
    static transformQueryToCanUseForm(query) {
        ...
        const leafMeasurePaths = ramda_1.default.pipe(ramda_1.default.map(m => {
            ...
            return leafMeasures;
        }), ramda_1.default.unnest, ramda_1.default.uniq)(measures);
        ...
        const hasPostAggregate = ramda_1.default.any(m => m.isPostAggregate(), query.measures);
        const leafMeasures = leafMeasurePaths.map(path => query.newMeasure(path));
        const leafMeasureAdditive = ramda_1.default.all(m => m.isAdditive(), leafMeasures);
        ...
        let filterDimensionsSingleValueEqual = this.collectFilterDimensionsWithSingleValueEqual(query.filters, dimensionsList.concat(segmentsList).reduce((map, d) => map.set(d, 1), new Map()));
    ...
    static timeDimensionsAsIs(timeDimensions) {
        return timeDimensions && ramda_1.default.sortBy(ramda_1.default.prop(0), timeDimensions.map(d => [d.expressionPath(), d.granularity])) || [];
    }
    ...
    static canUsePreAggregationForTransformedQueryFn(transformedQuery, refs) {
        ...
        const backAlias = (references) => references.map(r => (Array.isArray(r) ?
        ...
        const canUsePreAggregationNotAdditive = (references) => {
            const refTimeDimensions = backAlias(references.sortedTimeDimensions || sortTimeDimensions(references.timeDimensions));
            ...
            return ((transformedQuery.hasNoTimeDimensionsWithoutGranularity) && (!transformedQuery.hasCumulativeMeasures) && (ramda_1.default.equals(qryTimeDimensions, refTimeDimensions)) && (transformedQuery.isAdditive ||
                ramda_1.default.equals(transformedQuery.timeDimensions, refTimeDimensions)) && (filterDimensionsSingleValueEqual &&
                ...
                transformedQuery.allFiltersWithinSelectedDimensions &&
                    ramda_1.default.equals(backAliasSortedDimensions, transformedQuery.sortedDimensions)) && (ramda_1.default.all(m => backAliasMeasures.indexOf(m) !== -1, transformedQuery.measures) ||
                // TODO do we need backAlias here?
                ramda_1.default.all(m => backAliasMeasures.indexOf(m) !== -1, transformedQuery.leafMeasures)));
        };
        /**
         ...
         */
        const expandGranularity = (granularity) => (transformedQuery.granularityHierarchies[granularity] ||
            [granularity]);
        /**
        ...
        const windowGranularityMatches = (references) => {
            ...
            const sortedTimeDimensions = references.sortedTimeDimensions ||
                sortTimeDimensions(references.timeDimensions);
            return expandGranularity(transformedQuery.windowGranularity)
                .map(windowGranularity => ramda_1.default.all(td => td[1] === windowGranularity, sortedTimeDimensions))
                .filter(x => !!x)
                .length > 0;
        };
        /**
        ...
        const canUsePreAggregationLeafMeasureAdditive = (references) => {
            ...
            const dimensionsMatch = (dimensions, doBackAlias) => ramda_1.default.all(d => (doBackAlias ?
                ...
                (references.sortedDimensions || references.dimensions)).indexOf(d) !== -1, dimensions);
            const timeDimensionsMatch = (timeDimensionsList, doBackAlias) => ramda_1.default.allPass(timeDimensionsList.map(tds => ramda_1.default.anyPass(tds.map(td => ramda_1.default.contains(td)))))(doBackAlias ?
            ...
            if (transformedQuery.ungrouped) {
                const allReferenceCubes = ramda_1.default.pipe(ramda_1.default.map(m => (m.dimension || m).split('.')[0]), ramda_1.default.uniq, ramda_1.default.sortBy(ramda_1.default.identity))(references.measures.concat(references.dimensions).concat(references.timeDimensions));
         ...
         * @returns {boolean}
         */
        const canUseFn = (transformedQuery.leafMeasureAdditive && !transformedQuery.hasMultipliedMeasures && !transformedQuery.hasPostAggregate || transformedQuery.ungrouped) ? (r) => canUsePreAggregationLeafMeasureAdditive(r) ||
    ...
    buildRollupJoin(preAggObj, preAggObjsToJoin) {
        return this.query.cacheValue(['buildRollupJoin', JSON.stringify(preAggObj), JSON.stringify(preAggObjsToJoin)], () => {
            ...
            const existingJoins = ramda_1.default.unnest(preAggObjsToJoin.map(
            // TODO join hints?
    ...
    cubesFromPreAggregation(preAggObj) {
        return ramda_1.default.uniq(preAggObj.references.measures.map(m => this.query.cubeEvaluator.parsePath('measures', m)).concat(preAggObj.references.dimensions.map(m => this.query.cubeEvaluator.parsePathAnyType(m))).map(p => p[0]));
    }
    evaluatedPreAggregationObj(cube, preAggregationName, preAggregation, canUsePreAggregation) {
        ...
        if (preAggregation.type === 'rollupJoin') {
            ...
            // TODO evaluation optimizations. Should be cached or moved to compile time.
            const referencedPreAggregations = preAggObj.references.rollups.map(name => {
                const [referencedCube, referencedPreAggregation] = this.query.cubeEvaluator.parsePath('preAggregations', name);
                return this.evaluatedPreAggregationObj(referencedCube, referencedPreAggregation, this.query.cubeEvaluator.byPath('preAggregations', name), canUsePreAggregation);
            });
            if (referencedPreAggregations.length === 0) {
                throw new UserError_1.UserError(`rollupLambda '${cube}.${preAggregationName}' should reference at least on rollup`);
            }
            referencedPreAggregations.forEach((referencedPreAggregation, i) => {
                if (i === referencedPreAggregations.length - 1 && preAggObj.preAggregation.unionWithSourceData && preAggObj.cube !== referencedPreAggregations[i].cube) {
                    throw new UserError_1.UserError(`unionWithSourceData can be enabled only for pre-aggregation within '${preAggObj.cube}' cube but '${referencedPreAggregations[i].preAggregationName}' pre-aggregation is defined within '${referencedPreAggregations[i].cube}' cube`);
                }
    ...
    static checkPartitionGranularityDefined(cube, preAggregationName, preAggregation) {
        if (!preAggregation.preAggregation.partitionGranularity) {
            throw new UserError_1.UserError(`'${preAggregation.cube}.${preAggregation.preAggregationName}' referenced by '${cube}.${preAggregationName}' rollupLambda doesn't have partition granularity. Partition granularity is required if multiple rollups are provided.`);
        }
    ...
    rollupLambdaUnion(preAggregationForQuery, rollupGranularity) {
        if (!preAggregationForQuery.referencedPreAggregations) {
            return this.preAggregationTableName(preAggregationForQuery.cube, preAggregationForQuery.preAggregationName, preAggregationForQuery.preAggregation);
        }
        ...
        const columnsFor = (targetReferences, references, preAggregation) => Object.keys(targetReferences).map(member => `${references[this.query.cubeEvaluator.pathFromArray([preAggregation.cube, member.split('.')[1]])]} ${targetReferences[member]}`);
        const tables = preAggregationForQuery.referencedPreAggregations.map(preAggregation => {
            ...
            return {
                tableName: this.preAggregationTableName(preAggregation.cube, preAggregation.preAggregationName, preAggregation.preAggregation),
    ...
    rollupPreAggregation(preAggregationForQuery, measures, isFullSimpleQuery, filters) {
        ...
        const replacedFilters = filters || segmentFilters
