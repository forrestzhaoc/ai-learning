...
function factoryQueueDriver(cacheAndQueueDriver, queueDriverOptions) {
    switch (cacheAndQueueDriver || 'memory') {
        case 'redis':
            return new RedisQueueDriver_1.RedisQueueDriver(queueDriverOptions);
        case 'memory':
...
class QueryQueue {
    ...
    constructor(redisQueuePrefix, options) {
         ...
         */
        this.streamEvents = new events_1.EventEmitter();
    }
    /**
    ...
    async reconcileQueueImpl() {
        ...
        try {
            ...
            await Promise.all(ramda_1.default.pipe(ramda_1.default.filter(([queryKey, _queueId]) => {
            ...
            }), ramda_1.default.take(this.concurrency), ramda_1.default.map((([queryKey, queueId]) => this.sendProcessMessageFn(queryKey, queueId))))(toProcess));
        }
        finally {
            this.queueDriver.release(queueConnection);
        }
